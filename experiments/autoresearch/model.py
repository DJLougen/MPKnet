...
class MPKx(nn.Module):
    ...
    def __init__(self, num_classes: int = 10, ch: int = 48,
        ...
        # K pathway: stride=2 (intermediate), 1 layer — full ch width for richer cross-stream modulation
        self.K_block1 = StridedMonocularBlock(3, ch, kernel_size, stride=self.k_stride)
        ...
        # Bidirectional residual gating is identity-centered: 1 + alpha * tanh(gate).
        self.k_gate_mode = "bidirectional_tanh"
        self.k_gate_alpha = 0.5
        self.k_gate1_M_left = nn.Linear(ch, ch)
        self.k_gate1_M_right = nn.Linear(ch, ch)
        self.k_gate1_P_left = nn.Linear(ch, ch)
        self.k_gate1_P_right = nn.Linear(ch, ch)
        ...
        self.K_block2 = StridedMonocularBlock(ch, ch, kernel_size, stride=1)

        self.M_block2 = StridedMonocularBlock(ch, ch, kernel_size, stride=1)

        # K gates for block 2 (cross-stream modulation)
        self.k_gate2_M_left = nn.Linear(ch, ch)
        self.k_gate2_M_right = nn.Linear(ch, ch)
        self.k_gate2_P_left = nn.Linear(ch, ch)
        self.k_gate2_P_right = nn.Linear(ch, ch)

        # ========== V1 FUSION ==========
        # K is purely modulatory (gating), not a feature stream.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # K-gating 1: bidirectional residual modulation.
        # Multiplier range is [1-alpha, 1+alpha], allowing suppression and facilitation.
        k_ctx1_left = self.gap(K_left).flatten(1)
        k_ctx1_right = self.gap(K_right).flatten(1)
        M_left = M_left * (1 + self.k_gate_alpha * torch.tanh(self.k_gate1_M_left(k_ctx1_left))).unsqueeze(-1).unsqueeze(-1)
        M_right = M_right * (1 + self.k_gate_alpha * torch.tanh(self.k_gate1_M_right(k_ctx1_right))).unsqueeze(-1).unsqueeze(-1)
        P_left = P_left * (1 + self.k_gate_alpha * torch.tanh(self.k_gate1_P_left(k_ctx1_left))).unsqueeze(-1).unsqueeze(-1)
        P_right = P_right * (1 + self.k_gate_alpha * torch.tanh(self.k_gate1_P_right(k_ctx1_right))).unsqueeze(-1).unsqueeze(-1)
        ...
        # K-gating 2: same bidirectional residual form.
        k_ctx2_left = self.gap(K_left).flatten(1)
        k_ctx2_right = self.gap(K_right).flatten(1)
        M_left = M_left * (1 + self.k_gate_alpha * torch.tanh(self.k_gate2_M_left(k_ctx2_left))).unsqueeze(-1).unsqueeze(-1)
        M_right = M_right * (1 + self.k_gate_alpha * torch.tanh(self.k_gate2_M_right(k_ctx2_right))).unsqueeze(-1).unsqueeze(-1)
        P_left = P_left * (1 + self.k_gate_alpha * torch.tanh(self.k_gate2_P_left(k_ctx2_left))).unsqueeze(-1).unsqueeze(-1)
        P_right = P_right * (1 + self.k_gate_alpha * torch.tanh(self.k_gate2_P_right(k_ctx2_right))).unsqueeze(-1).unsqueeze(-1)
        ...
        # Combine four streams (binocular M and P only; K is gate-only)
