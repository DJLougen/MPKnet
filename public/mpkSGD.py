...
class MPKNet(nn.Module):
    def __init__(self, num_classes=100, ch=64, hid=128, use_prefilters=True,
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(ch*2 + ch//4, hid, 1, bias=False), nn.BatchNorm2d(hid), nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        if self.use_cellpop:
            # 3x3 structured downsample per pathway; spatial dims reduced by /3
        ...
        # Align spatial sizes before fuse
        target_hw = P.shape[2:]
        ...
        z = self.fuse(torch.cat([P, M, K], dim=1))
        return self.head(z)
