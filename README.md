...
This project is a side project to my PhD research on the LGN at the University of Toronto. I had the idea a couple years ago and built it over the summer of 2025. After reviewing prior attempts at bio-inspired neural networks, I found that existing approaches often fail to capture the organizational principles of biological visual systems. Many borrow surface-level inspiration—such as Gabor filters—without modeling the fundamental parallel-stream architecture that evolution has conserved across mammals. Other approaches, like EEG-guided training, reveal correlations between neural activity and image processing but focus more on pattern matching brain signals than on capturing the underlying structure of biological vision. Similarly, models that replicate cellular firing patterns raise a deeper question: does achieving similar output necessarily mean the model operates through the same mechanism? This project seeks a middle ground between neuroscience and application—one that takes biological structure seriously while remaining practically useful.
